...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.sprint_blocked = False  # Prevents sprinting until shift is released after stamina depletion
        # crouch
        self.crouch_speed = 40

    def ui_update(self, surface):
        sprint_slider_outline = pygame.Rect(8, display_height - 16, 34, 6)
        pygame.draw.rect(surface, (100, 100, 100), sprint_slider_outline)
        sprint_slider_bg = pygame.Rect(9, display_height - 15, 32, 4)
        pygame.draw.rect(surface, (192, 192, 192), sprint_slider_bg)
        sprint_slider = pygame.Rect(9, display_height - 15, (self.current_sprint_stamina / self.max_sprint_stamina) * 32, 4)
        pygame.draw.rect(surface, (255, 255, 0), sprint_slider)
    ...
    def update(self, dt, walls, boxes, doors):
        ...
        # Reset sprint_blocked when shift key is released
        if not is_sprinting:
            self.sprint_blocked = False

        # Set speed based on state (Sprint > Walk)
        can_sprint = is_sprinting and self.moving and self.current_sprint_stamina > 0 and not self.sprint_blocked
        ...
        if can_sprint:
            ...
            # Block sprinting if stamina reaches zero
            if self.current_sprint_stamina <= 0:
                self.sprint_blocked = True
        else:
